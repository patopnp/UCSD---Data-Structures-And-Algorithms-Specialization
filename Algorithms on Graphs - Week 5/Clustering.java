...
public class Clustering {
    ...
    public double primClustering(ArrayList<Integer>[] adj, ArrayList<Double>[] cost, int numPartitions) {

        double[] dist = new double[adj.length];

        double[] largestShortesDistanceToTree = new double[numPartitions-1];

        for (int i = 0; i < numPartitions-1; i++)
            largestShortesDistanceToTree[i] = Double.MIN_VALUE;
        ...
        while (h.isEmpty() == false) {
            ...
            if (u.dist > largestShortesDistanceToTree[0]) {
                largestShortesDistanceToTree[0] = u.dist;
                int i = 1;
                while (i < largestShortesDistanceToTree.length && u.dist > largestShortesDistanceToTree[i]) {
                    largestShortesDistanceToTree[i - 1] = largestShortesDistanceToTree[i];
                    largestShortesDistanceToTree[i] = u.dist;
                    i++;
                }
            }
        ...
        return largestShortesDistanceToTree[0];
    }

    private static double clustering(int[] x, int[] y, int numPartitions) {
        ...
        result = clusteringAlgorithm.primClustering(adj, cost, numPartitions);
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(clustering(x, y, numPartitions));
    }
}
